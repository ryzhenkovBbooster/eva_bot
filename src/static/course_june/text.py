...
def start_message(name, date):
    ...
    return f"Добрый день, @{name}! Рады приветствовать вас в нашей команде! Меня зовут Ева, я - телеграм бот, и буду помогать вам" \
                " адаптироваться на тестовом периоде.Также хотела бы представить вам Дениса Кравчук @DenKravchuk - специалиста" \
                " по адаптации, который также будет подключаться для ответов на вопросы, если они у вас возникнут." \
                " Стандартное время работы в компании с 9:00 до 18:00 по часовому поясу UTC+3 (Турция, Мадагаскар, Катар)." \
                f" Мне передали информацию, что {date} вы готовы приступить, верно?\n" \
           f"Ответь мне на это сообщение 'готов', и мы начнем работать вместе!"


def info_message(name):
    return f"@{name} Уведомления в чатах нашей компании выключены, поэтому," \
           " чтобы я мог оперативно реагировать на ваши сообщения, пожалуйста," \
           " пишите через @логин в тексте сообщения или используйте функцию 'ответить', чтобы я ничего не упустил."


##                                     5.  АКН подготавливает файл с практическими заданиями  для стажера:
def create_practical_tasks(fullname, link):
    return f"Привет, сотрудник {fullname} вышел на тестовый период," \
           " в течении дня он пройдет первый этап курса адаптации," \
           f" далее во втором этапе ему нужно будет выполнить {link}," \
           " которые покажут его квалификацию. Заполни, пожалуйста, поля “Задание”" \
           " и “Крайний срок”, они должны быть рассчитаны на 3 дня.  Задачи должны быть составлены так," \
           " чтобы показать подходит ли нам этот кандидат"
...
def generate_mail(login, passwd):
    return "Войдите в вашу корпоративную почту https://www.google.com/intl/ru/gmail/about/\n" \
           f"Логин: {login}\n" \
           f"Пароль: {passwd}\n" \
           "После входа, смените пароль и сообщите о входе в почту ответным сообщением 'почта создана'.  "



def reg_to_skill_up():
    return "Для первичной адаптации в нашей компании мы используем платформу Skill-up." \
           " Доступ на платформу был отправлен вам на почту. В разделе 'Обучение' вы найдете Курс новичка," \
           " который поможет вам освоить основные правила компании, определит ваши компетенции и поможет подготовить" \
           " ваш компьютер к полноценной работе у нас. Курс необходимо пройти в течении 5 дней," \
           " поэтому можете приступать к прохождению!"
...
def rename_email_static(name, additional_email, oldEmail, username):
    return f" {name}, привет! Я переименовала вашу почту на должностную и открыла доступы." \
           " Теперь вам доступны все файле вашего департамента на Google Диске!" \
           f" Новая почта {additional_email} - логин почты, пароль остался прежний." \
           f" Предыдущая почта, '{oldEmail}', сохранена в вашем аккаунте и связана с новым адресом." \
           f" Важно помнить, что все регистрации в сервисах по-прежнему должны осуществляться через '{oldEmail}'." \
           f" И будущие регистрации в сервисах компании также следует создавать с использованием '{oldEmail}'." \
           f"Если возникнут трудности с доступом или потребуется техническая помощь, не стесняйтесь обращаться к @{username}, он всегда готов помочь вам!" \
...
def invate_to_meeting_is(june):
    return f'@{june} Также, для вашей успешной адаптации в компании мы' \
           ...
           f' UTC+3. Вот ссылка чата сотрудников (), в течении дня я добавлю эту встречу в календарь. ' \
...
def finaly_is_from_chat_with_user(june):
    return f'@{june}, хочу сообщить об успешном завершение' \
           ' испытательного срока, поздравляю вас' \
           '  с официальным вступлением в должность!' \
